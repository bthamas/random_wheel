...
class WheelOfFortune {
    ...
    private closeStats: HTMLButtonElement;
    ...
    private init(): void {
        ...
        this.itemInput = document.getElementById('itemInput') as HTMLInputElement;
        ...
        this.closeStats = document.getElementById('closeStats') as HTMLButtonElement;
    ...
    private bindEvents(): void {
        // Add item events
        this.itemInput.addEventListener('keypress', (e: KeyboardEvent) => {
            if (e.key === 'Enter') {
                this.addItem();
            }
        });
        ...
        this.statsModal.addEventListener('click', (e: MouseEvent) => {
    ...
    private addSampleItems(): void {
        const sampleItems: string[] = ['Pizza', 'Hamburger', 'Sushi', 'Saláta', 'Pasta', 'Steak'];
    ...
    private updateItemsList(): void {
        ...
        this.items.forEach(item => {
            ...
            itemTag.className = 'bg-gradient-to-r from-purple-500 to-purple-700 text-white px-4 py-2 rounded-full text-sm flex items-center gap-2 animate-slide-in shadow-lg';
            itemTag.innerHTML = `
                <span>${item}</span>
                <button class="bg-white/30 border-none text-white w-5 h-5 rounded-full cursor-pointer flex items-center justify-center text-xs transition-all duration-300 hover:bg-white/50 hover:scale-110" onclick="wheelApp.removeItem('${item}')">
    ...
    private spinWheel(): void {
        ...
        // Calculate which item will win (pointer is at 0 degrees right side)
        // The wheel segments start from -90 degrees (top), so we need to adjust
        const segmentAngle = 360 / this.items.length;
        
        // The pointer is at 0 degrees (right side)
        // After rotation, we need to find which segment is at 0 degrees
        // Since segments start at -90 degrees, we add 90 to align
        const normalizedAngle = (finalAngle + 90) % 360;
        
        // Calculate which segment this corresponds to
        const winningIndex = Math.floor(normalizedAngle / segmentAngle) % this.items.length;
    ...
    private highlightWinningSegment(winner: string): void {
        ...
        if (winnerIndex >= 0) {
            const winningSegment = this.wheel.querySelector(`[data-index="${winnerIndex}"]`) as HTMLElement;
            if (winningSegment) {
    ...
    private updateStatistics(winner: string): void {
    ...
    private updateStatisticsDisplay(): void {
        ...
        (document.getElementById('totalSpins') as HTMLElement).textContent = this.statistics.totalSpins.toString();
        (document.getElementById('uniqueItems') as HTMLElement).textContent = this.items.length.toString();
        ...
        if (sortedStats.length > 0) {
            (document.getElementById('mostFrequent') as HTMLElement).textContent = `${sortedStats[0][0]} (${sortedStats[0][1].wins}x)`;
            (document.getElementById('leastFrequent') as HTMLElement).textContent = `${sortedStats[sortedStats.length - 1][0]} (${sortedStats[sortedStats.length - 1][1].wins}x)`;
        }
        
        // Update detailed stats
        const detailedStats = document.getElementById('detailedStats') as HTMLElement;
    ...
    private initShakeDetection(): void {
    ...
    // Public method for removing items (called from HTML)
...
document.addEventListener('DOMContentLoaded', () => {
    (window as any).wheelApp = new WheelOfFortune();
});
...
document.addEventListener('touchend', (e: TouchEvent) => {
    ...
    if (Math.abs(diffX) > Math.abs(diffY) && Math.abs(diffX) > 50) {
        ...
        const wheelApp = (window as any).wheelApp as WheelOfFortune;
        if (wheelApp && !wheelApp['isSpinning'] && wheelApp['items'].length > 0) {
            wheelApp['spinWheel']();
        }
    }
